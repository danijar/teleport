...
class TestServer:
  ...
  def test_single_client(self, Server, addr):
    addr = addr.format(port=teleport.get_free_port())
    def function(data):
    ...
    with server:
      client = teleport.Client(addr, pings=0, maxage=1)
  ...
  def test_multiple_clients(self, Server, addr):
    ...
    with server:
      clients = []
      for i in range(10):
        client = teleport.Client(addr, i, pings=0, maxage=1)
        client.connect()
  ...
  def test_multiple_methods(self, Server, addr):
    ...
    with server:
      client = teleport.Client(addr, pings=0, maxage=0.1)
      client.connect(retry=False, timeout=1)
  ...
  def test_connect_before_server(self, Server, addr):
    ...
    def client():
      ...
      result = future.result()
      results.put(result)
    thread = teleport.Thread(client, start=True)
    barrier.wait()
    time.sleep(0.2)
    with server:
  ...
  def test_future_order(self, Server, addr):
    ...
    server.bind('function', lambda data: data)
    with server:
      client = teleport.Client(addr, 0, pings=0, maxage=1)
      client.connect(retry=False, timeout=1)
  ...
  def test_future_cleanup_errors(self, Server, addr):
    ...
    server.bind('function', lambda data: data)
    with server:
      client = teleport.Client(addr, 0, pings=0, maxage=1, errors=True)
      client.connect(retry=False, timeout=1)
      client.function({'foo': 1})
      client.function({'foo': 2})
  ...
  def test_ping_alive(self, Server, addr):
    ...
    with server:
      client = teleport.Client(addr, pings=0.01, maxage=0.05)
      client.connect()
  ...
  def test_ping_dead(self, Server, addr):
    ...
    server.bind('function', slow)
    with server:
      client = teleport.Client(addr, pings=0.1, maxage=0.01)
      client.connect()
      with pytest.raises(teleport.NotAliveError):
  ...
  def test_remote_error(self, Server, addr):
    ...
    with server:
      client = teleport.Client(addr, errors=False, connect=True)
      future = client.function({'bar': 0})
      with pytest.raises(teleport.RemoteError) as info1:
        future.result()
      time.sleep(0.1)
  ...
  def test_remote_client_errors(self, Server, addr):
    ...
    server.bind('function', error)
    with server:
      client = teleport.Client(addr, connect=True, errors=True)
      client.function({'foo': 1})
      time.sleep(0.2)
      assert len(client.queue) == 1
      client.queue[0].check()
      assert client.queue[0].done()
      with pytest.raises(teleport.RemoteError) as info:
        client.function({'foo': 2})
  ...
  def test_donefn_no_backlog(self, Server, addr, workers):
    ...
    with server:
      client = teleport.Client(addr, pings=0, maxage=1, connect=True)
  ...
  def test_proxy(self, Server, inner_addr, outer_addr, workers):
    inner_addr = inner_addr.format(port=teleport.get_free_port())
    outer_addr = outer_addr.format(port=teleport.get_free_port())
    proxy_client = teleport.Client(inner_addr)
    ...
    with server:
      ...
      with proxy_server:
        client = teleport.Client(outer_addr, pings=0, maxage=1)
        client.connect(retry=False, timeout=1)
